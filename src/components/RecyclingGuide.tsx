...
export const RecyclingGuideComponent: React.FC<RecyclingGuideProps> = ({ onAskAi }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Filter & Search Bar */}
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 bg-white p-4 rounded-[24px] border border-[#1B3022]/10 shadow-2xs">
        {/* Search Input */}
        <div className="relative w-full sm:w-64">
          <Search className="w-4 h-4 text-[#1B3022]/40 absolute left-3.5 top-3" />
          <input
            type="text"
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            placeholder="Search items (e.g., bottle, glass)..."
            className="w-full pl-9 pr-4 py-2 bg-[#F7F8F3] border border-[#1B3022]/15 rounded-full text-xs text-[#1B3022] placeholder-[#1B3022]/40 focus:outline-none focus:border-[#A7C957] italic"
          />
        </div>
        ...
        <div className="flex items-center space-x-1.5 overflow-x-auto w-full sm:w-auto scrollbar-none">
          {categories.map((cat) => (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider whitespace-nowrap transition ${
                selectedCategory === cat
                  ? 'bg-[#1B3022] text-[#F7F8F3]'
                  : 'bg-[#F7F8F3] text-[#1B3022]/70 border border-[#1B3022]/10 hover:bg-[#E8EDE0]'
              }`}
            >
              {cat}
            </button>
        ...
        {filteredItems.map((item) => (
          <div
            ...
            className={`bg-white rounded-[24px] p-6 border shadow-2xs space-y-4 flex flex-col justify-between ${
              item.isRecyclable ? 'border-[#1B3022]/10' : 'border-amber-300/40'
            }`}
          >
            <div>
              <div className="flex items-center justify-between mb-3">
                <span className="text-[10px] font-bold uppercase tracking-wider text-[#1B3022]/50 bg-[#E8EDE0] px-3 py-1 rounded-full border border-[#1B3022]/10">
                  {item.category}
                </span>

                {item.isRecyclable ? (
                  <span className="inline-flex items-center space-x-1 text-[10px] font-bold uppercase tracking-wider text-[#1B3022] bg-[#A7C957]/30 border border-[#1B3022]/10 px-3 py-1 rounded-full">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#6A994E]" />
                    <span>Curbside Recyclable</span>
                  </span>
                ) : (
                  <span className="inline-flex items-center space-x-1 text-[10px] font-bold uppercase tracking-wider text-amber-900 bg-amber-50 border border-amber-200 px-3 py-1 rounded-full">
              ...
              <h3 className="editorial-serif text-2xl font-normal italic text-[#1B3022] flex items-center space-x-2">
                <span>{item.name}</span>
                {item.recyclingCode && (
                  <span className="text-xs font-mono bg-[#E8EDE0] text-[#1B3022] px-2 py-0.5 rounded-full font-bold not-italic">
                    {item.recyclingCode}
                  </span>
                )}
              </h3>

              <p className="text-xs text-[#1B3022]/75 mt-2 leading-relaxed">{item.instructions}</p>

              {item.doNotInclude && (
                <div className="mt-3 text-[11px] text-rose-900 bg-rose-50 p-3 rounded-[16px] border border-rose-100">
            ...
            <div className="pt-3 border-t border-[#1B3022]/10 flex items-center justify-between">
              <button
                ...
                className="text-xs font-bold text-[#1B3022] hover:text-[#6A994E] flex items-center space-x-1.5 uppercase tracking-wider text-[10px]"
              >
                <Bot className="w-3.5 h-3.5 text-[#6A994E]" />
